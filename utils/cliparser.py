...
# Target options
target = OptionGroup(
    parser, "Target", "These options have to be provided, to define the target URL. "
)

target.add_option("-u", "--url", action="store", dest="url", help="Target URL.")

# Request options
request = OptionGroup(
    parser,
    "Request",
    "These options have how to connect and where to inject to the target URL.",
)
...
detection = OptionGroup(
    parser, "Detection", "These options can be used to customize the detection phase."
)
...
oscmd.add_option("--os-cmd", dest="os_cmd", help="Execute an operating system command.")
