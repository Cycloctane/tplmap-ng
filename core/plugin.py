...
class Plugin(object):
    ...
    def rendered_detected(self):
        ...
        if test_os_code and test_os_code_expected:
            ...
            if os and re.search(test_os_code_expected, os):
                ...
                if (
                    test_cmd_code
                    and test_cmd_code_expected
                    and test_cmd_code_expected == self.execute(test_cmd_code)
                ):
    ...
    def detect(self):
        ...
        if "R" in techniques:
            ...
            # Else, print and execute rendered_detected()
                ...
                log.info(
                    "%s plugin has confirmed injection with tag '%s%s%s'"
                    % (
                        self.plugin,
                        repr(prefix).strip("'"),
                        repr(render).strip("'"),
        ...
        if expected in self.render(
            ...
            if not self.get("unreliable_render"):
                log.info(
                    ...
                    % (self.plugin, repr(render_action.get("render") % ({"code": "*"})))
                )
    ...
    def _detect_blind(self):
        ...
        for prefix, suffix in self._generate_contexts():
            ...
            if getattr(self, call_name)(
                code=payload_false, prefix=prefix, suffix=suffix, blind=True
            ):
                continue
    ...
    def _detect_render(self):
        ...
        # Print what it's going to be tested
        log.info(
            "%s plugin is testing rendering with tag %s"
            % (
                self.plugin,
    ...
    def write(self, data, remote_path):
        ...
        if (
            not action
            or not payload_write
            or not payload_truncate
            or not call_name
            or not hasattr(self, call_name)
        ):
            return
    ...
    def evaluate_blind(self, code, **kwargs):
        ...
        if "%(code_b64)s" in payload_action:
            ...
            execution_code = payload_action % ({"code": code, "delay": expected_delay})

        return getattr(self, call_name)(
            code=execution_code, prefix=prefix, suffix=suffix, blind=True
        )
    ...
    def bind_shell(self, port, shell="/bin/sh"):
        ...
        if (
            not action
            or not isinstance(payload_actions, list)
            or not call_name
            or not hasattr(self, call_name)
        ):
            return
