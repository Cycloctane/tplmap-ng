...
from core.clis import MultilineShell, Shell
...
def detect_template_injection(channel, plugins=plugins):
    ...
    for i in range(len(channel.injs)):
        ...
        for plugin in plugins:
            ...
            if (
                channel.args.get("engine")
                and channel.args.get("engine").lower() != current_plugin.plugin.lower()
            ):
                continue
