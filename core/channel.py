...
class Channel:
    ...
    def _parse_url(self):
        ...
        for index in [
            i
            for i in range(url_path_base_index, url_path_base_index + len(url_path))
    ...
    def _parse_header(self, all_injectable=False):
        for param_value in self.args.get("headers", []):
            ...
            if self.tag in value or all_injectable:
                self.injs.append(
                    {"field": "Header", "part": "value", "value": value, "param": param}
                )
    ...
    def _parse_get(self, all_injectable=False):
        params_dict_list = urlparse.parse_qs(
            urlparse.urlsplit(self.url).query, keep_blank_values=True
        )
